...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true); // Removed * 0.7f scaling

    const float square_sz = GetFrameHeight() * 0.6f;
    const ImVec2 pos = window->DC.CursorPos;

    // Calculate total width for the item (label + spacing + checkbox)
    const float total_width = CalcItemWidth();
    const float total_height = ImMax(label_size.y, square_sz) + style.FramePadding.y * 2.0f;
    const ImRect total_bb(pos, pos + ImVec2(total_width, total_height));

    ItemSize(total_bb, style.FramePadding.y);

    static std::map<ImGuiID, float> map;
    auto anim = map.find(id);
    if (anim == map.end()) {
        map.insert({ id, 0.0f });
        anim = map.find(id);
    }

    anim->second = ImLerp(anim->second, *v ? 1.0f : 0.0f, 0.2f);
    const float animation_threshold = 0.09f;
    if (anim->second < animation_threshold)
        anim->second = 0.0f;

    bool is_visible = ItemAdd(total_bb, id);
    ...
    // Calculate checkbox bounding box (right-aligned)
    const ImRect check_bb(
        total_bb.Max.x - square_sz,
        total_bb.Min.y + style.FramePadding.y,
        total_bb.Max.x,
        total_bb.Min.y + style.FramePadding.y + square_sz
    );
    ...
    if (mixed_value) {
        ...
        draw_list->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad,
            GetColorU32(ImGuiCol_CheckMark),
            style.FrameRounding);
    }
    else if (is_visible) {
        draw_list->AddRectFilledMultiColor(check_bb.Min, check_bb.Max,
            GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_FrameBg),
            GetColorU32(ImGuiCol_FrameBg), GetColorU32(ImGuiCol_FrameBg));

        if (anim->second > animation_threshold) {
            float second_checkbox_size = 14.0f, scale = second_checkbox_size / 18.0f,
                anim_size = (square_sz * scale) * anim->second;
            ImVec2 center = check_bb.GetCenter();
            ImVec2 half_size = ImVec2(anim_size * 0.5f, anim_size * 0.5f);
            ImVec2 rect_min = center - half_size;
            ImVec2 rect_max = center + half_size;

            draw_list->AddRectFilled(rect_min, rect_max, GetColorU32(ImGuiCol_CheckMark));
        }
        draw_list->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_Border), 0.0f);
    }

    // Calculate label position (left-aligned)
    const ImVec2 label_pos(total_bb.Min.x, total_bb.Min.y + style.FramePadding.y);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float total_width = CalcItemWidth();
    const float slider_height = g.FontSize * 0.6f;
    const float value_width = CalcTextSize(value_buf).x;
    const float interactive_width = total_width - (label_size.x > 0.0f ? label_size.x + style.ItemInnerSpacing.x : 0.0f);

    ImVec2 pos = window->DC.CursorPos;

    // Calculate total bounding box for the item (label + spacing + slider)
    const float total_height = ImMax(label_size.y, slider_height) + style.FramePadding.y * 2.0f;
    const ImRect total_bb(pos, pos + ImVec2(total_width, total_height));
    ...
    // Calculate frame bounding box (right-aligned)
    const ImRect frame_bb(
        total_bb.Min.x + (total_width - interactive_width),
        total_bb.Min.y + style.FramePadding.y,
        total_bb.Max.x,
        total_bb.Min.y + style.FramePadding.y + slider_height
    );
    ...
    ImVec4 colFrame = GetStyleColorVec4(ImGuiCol_FrameBg);
    ...
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, colFrame, 1.0f / 0.005f * ImGui::GetIO().DeltaTime);
    it_anim->second.Grab = ImLerp(it_anim->second.Grab, colGrab, 1.0f / 0.005f * ImGui::GetIO().DeltaTime);
    ...
    {
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(style.FrameBorderSize, style.FrameBorderSize) * 2, grab_bb.Max + grab_padding - ImVec2(style.FrameBorderSize, style.FrameBorderSize) * 2, GetColorU32(ImGuiCol_SliderGrabActive), style.FrameRounding);
    }

    ImU32 border_col = GetColorU32(ImGuiCol_Border);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, border_col, style.FrameRounding);

    // Render value (right-aligned within the slider frame)
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - value_width - style.FramePadding.x, frame_bb.Min.y + style.FramePadding.y), GetColorU32(ImGuiCol_TextDisabled), value_buf);

    // Render label (left-aligned)
    if (label_size.x > 0.0f) {
        ImGui::PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TitleBg]);
        ImGui::RenderText(total_bb.Min + ImVec2(1, 1), label);
        ImGui::RenderText(total_bb.Min + ImVec2(-1, -1), label);
        ImGui::RenderText(total_bb.Min + ImVec2(-1, 1), label);
        ImGui::RenderText(total_bb.Min + ImVec2(1, -1), label);
        ImGui::PopStyleColor();
