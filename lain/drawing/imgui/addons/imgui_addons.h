...
namespace ImAdd
{
    // Helper struct for animations
    struct stColors_State {
        ImVec4 Frame;
        ImVec4 Border; // Used by Selectable, FrameRadioIcon, FrameRadio
        ImVec4 Text;   // Used by BeginCombo, FrameRadioIcon, FrameRadio
    };
    ...
    bool    SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.0f", float width = -0.1f);
    ...
    bool    MultiCombo(const char* label, std::vector<int>* selected, std::vector<const char*> items, float width = -0.1f, bool* value_changed_out = nullptr);

    bool    BeginChild(const char* label, const ImVec2& size = ImVec2(0, 0));
    void    EndChild();

	bool	KeyBind(const char* str_id, int* k, float custom_width = 0);
	bool    ColorEdit4(const char* label, float col[4]);
    void    ClearWidgetStates(); // New function to clear static states
}
