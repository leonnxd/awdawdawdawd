...
namespace ImAdd {
    // Global static maps for widget states
    static std::map<ImGuiID, bool> g_is_dragging;
    static std::map<ImGuiID, bool> g_show_input;
    static std::map<ImGuiID, char[128]> g_input_buffer;
    static std::map<ImGuiID, float> g_current_value;
    static std::map<ImGuiID, float> g_target_value;
    static std::map<ImGuiID, ImVec4> g_color_animations;
    static std::map<ImGuiID, ImAdd::stColors_State> g_anim_states; // Using the globally defined stColors_State
...
bool ImAdd::Button(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Animations
    auto it_anim = g_anim_states.find(id);

    if (it_anim == g_anim_states.end())
    {
        g_anim_states.insert({ id, ImAdd::stColors_State() });
        it_anim = g_anim_states.find(id);
...
bool ImAdd::CheckBox(const char* label, bool* v)
{
    ...
    const float total_width = checkbox_size + style.ItemInnerSpacing.x * 0.8f + label_size.x;
    const float total_height = ImMax(checkbox_size, label_size.y);
    const ImRect bb(pos, pos + ImVec2(total_width, total_height));

    ItemSize(bb.GetSize()); // Use the total size for item layout
    if (!ItemAdd(bb, id))
        return false;
    ...
    if (g_color_animations.find(id) == g_color_animations.end()) {
        g_color_animations[id] = target_color;
    }

    const float animation_speed = 1.0f / IMADD_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime;
    g_color_animations[id] = ImLerp(g_color_animations[id], target_color, animation_speed);

    // Render checkbox
    RenderNavHighlight(bb, id);

    // Gradient checkbox background
    const ImU32 base_color = GetColorU32(g_color_animations[id]);
    const ImU32 top_color = ImGui::ColorConvertFloat4ToU32(
        ImVec4(
            g_color_animations[id].x + 0.1f,
            g_color_animations[id].y + 0.1f,
            g_color_animations[id].z + 0.1f,
            g_color_animations[id].w
        )
    );
    const ImU32 bottom_color = ImGui::ColorConvertFloat4ToU32(
        ImVec4(
            g_color_animations[id].x - 0.05f,
            g_color_animations[id].y - 0.05f,
            g_color_animations[id].z - 0.05f,
            g_color_animations[id].w
        )
    );
    ...
    window->DrawList->AddRectFilledMultiColor(
        pos,
        pos + ImVec2(checkbox_size, checkbox_size), // Use explicit size for checkbox rendering
...
bool CustomSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ...
    if (right_clicked) {
        ImAdd::g_show_input[id] = true;
        // Initialize input buffer with current value
        ImGui::DataTypeFormatString(ImAdd::g_input_buffer[id], sizeof(ImAdd::g_input_buffer[id]), data_type, p_data, format);
    }

    // Start dragging on left click
    if (make_active && !ImAdd::g_is_dragging[id]) {
        ImAdd::g_is_dragging[id] = true;
        ImGui::SetActiveID(id, window);
    }

    // Stop dragging when mouse is released (anywhere, not just on the slider)
    if (ImAdd::g_is_dragging[id] && !ImGui::IsMouseDown(0)) {
        ImAdd::g_is_dragging[id] = false;
        ImGui::ClearActiveID();
    }

    ImVec4 colFrame = ImGui::GetStyleColorVec4(ImAdd::g_is_dragging[id] ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ImVec4 colGrab = ImGui::GetStyleColorVec4(ImAdd::g_is_dragging[id] ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);

    if (ImAdd::g_current_value.find(id) == ImAdd::g_current_value.end()) {
        ImAdd::g_current_value[id] = (data_type == ImGuiDataType_S32) ? (float)(*(int*)p_data) : *(float*)p_data;
        ImAdd::g_target_value[id] = ImAdd::g_current_value[id];
    }
    ...
    if (ImAdd::g_show_input[id]) {
        ImGui::OpenPopup(popup_id);
        ImAdd::g_show_input[id] = false;
    }

    if (ImGui::BeginPopup(popup_id)) {
        ImGui::Text("Enter value:");
        ImGui::SetKeyboardFocusHere();
        if (ImGui::InputText("##input", ImAdd::g_input_buffer[id], sizeof(ImAdd::g_input_buffer[id]), ImGuiInputTextFlags_EnterReturnsTrue)) {
            // Parse the input value
            if (data_type == ImGuiDataType_S32) {
                int new_val = atoi(ImAdd::g_input_buffer[id]);
                int min_val = *(int*)p_min;
                int max_val = *(int*)p_max;
                new_val = ImClamp(new_val, min_val, max_val);
                *(int*)p_data = new_val;
                ImAdd::g_current_value[id] = (float)new_val;
                ImAdd::g_target_value[id] = (float)new_val;
            }
            else {
                float new_val = (float)atof(ImAdd::g_input_buffer[id]);
                float min_val = *(float*)p_min;
                float max_val = *(float*)p_max;
                new_val = ImClamp(new_val, min_val, max_val);
                *(float*)p_data = new_val;
                ImAdd::g_current_value[id] = new_val;
                ImAdd::g_target_value[id] = new_val;
            }
            value_changed = true;
    ...
    if (ImAdd::g_is_dragging[id]) {
        ...
        if (data_type == ImGuiDataType_S32) {
            ...
            ImAdd::g_target_value[id] = new_value;
        }
        else {
            float min_val = *(float*)p_min;
            float max_val = *(float*)p_max;
    ...
    ImAdd::g_current_value[id] = ImLerp(ImAdd::g_current_value[id], ImAdd::g_target_value[id], 1.0f / 0.1f * ImGui::GetIO().DeltaTime);

    if (data_type == ImGuiDataType_S32) {
        int new_val = (int)roundf(ImAdd::g_current_value[id]);
        int min_val = *(int*)p_min;
        int max_val = *(int*)p_max;
        new_val = ImClamp(new_val, min_val, max_val);
        ...
        float new_val = ImAdd::g_current_value[id];
        float min_val = *(float*)p_min;
        float max_val = *(float*)p_max;
        new_val = ImClamp(new_val, min_val, max_val);
    ...
    ImVec2 grab_padding = ImVec2(grab_bb.Min.y - frame_bb.Min.y + 5.0f, 5.0f);
    grab_padding.y = grab_padding.x;

    RenderNavHighlight(frame_bb, id);
    // Render the gray background to match the size of the blue grab area
    RenderFrame(frame_bb.Min, frame_bb.Max + grab_padding - ImVec2(style.FrameBorderSize, style.FrameBorderSize), GetColorU32(colFrame), true, g.Style.FrameRounding);
    ...
    ImVec2 text_pos = pos + ImVec2(w / 2 - CalcTextSize(value_buf).x / 2, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0);
    for (int x = -1; x <= 1; x += 2) {
        for (int y = -1; y <= 1; y += 2) {
...
bool ImAdd::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ...
    if (ImAdd::g_is_dragging[id] && !IsMouseDown(0)) {
        ImAdd::g_is_dragging[id] = false;
        ClearActiveID();
    }

    ImVec4 colFrame = GetStyleColorVec4(ImAdd::g_is_dragging[id] ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ImVec4 colGrab = GetStyleColorVec4(ImAdd::g_is_dragging[id] ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);

    if (ImAdd::g_current_value.find(id) == ImAdd::g_current_value.end()) {
        ImAdd::g_current_value[id] = *((float*)p_data);
        ImAdd::g_target_value[id] = *((float*)p_data);
    }
    ...
    ImAdd::g_current_value[id] = ImLerp(ImAdd::g_current_value[id], ImAdd::g_target_value[id], 1.0f / IMADD_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
...
bool ImAdd::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, float width)
{
...
bool ImAdd::MultiCombo(const char* label, std::vector<int>* selected, std::vector<const char*> items, float width, bool* value_changed_out)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    bool value_changed = false;
    if (value_changed_out) *value_changed_out = false;
    ...
    // Ensure selection array matches items so preview/interaction stay in sync
    if (selected->size() < items.size())
        selected->resize(items.size(), 0);
    ...
    // Count selections and remember the last one to trim the trailing comma
    ...
    // Rebuild preview list; default to "None" when nothing is selected
    if (selected_count > 0)
    {
        preview.clear();
        for (int i = 0; i < items.size(); i++)
        {
            if ((*selected)[i] == 1)
            {
                preview += items[i] + std::string(i == last_selected ? "" : ", ");
            }
        }
    }
    ...
    if (BeginPopup(std::string("MultiCombo_" + std::string(label)).c_str(), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        for (int i = 0; i < items.size(); i++)
        {
            if (ImAdd::Selectable(items[i], (*selected)[i] == 1, ImVec2(size.x - style.WindowPadding.x * 2, 0)))
            {
                ...
                value_changed = true;
                if (value_changed_out) *value_changed_out = true;
            }
        }
...
bool ImAdd::BeginChild(const char* label, const ImVec2& size_arg)
{
    ...
    ImGui::BeginChild(label, size_arg, ImGuiChildFlags_None, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
    {
     ...
     //   pDrawList->AddLine(pos + ImVec2(2, 3), pos + ImVec2(size.x - 2, 3), ImGui::GetColorU32(ImGuiCol_SliderGrabActive));
        ...
        ImGui::BeginChild(string(label + string("#Main")).c_str(), ImVec2(), ImGuiChildFlags_None, NULL);
    }
    return true; // ImGui::BeginChild returns bool
}
...
bool ImAdd::ColorEdit4(const char* label, float col[4])
{
   ...
   // window->DrawList->AddRectFilledMultiColor(pos + ImVec2(1, 1), pos + ImVec2(GetFontSize() * 2, GetFontSize()) - ImVec2(1, 1), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow, 0), GetColorU32(ImGuiCol_WindowShadow), GetColorU32(ImGuiCol_WindowShadow));
    ...
    if (BeginPopup(std::string(std::string(label) + "##Picker").c_str()))
    {
        ...
        // Hex input field (RGBA) under the picker
        char buf[16];
        int r = (int)IM_F32_TO_INT8_SAT(col[0]);
        int g = (int)IM_F32_TO_INT8_SAT(col[1]);
        int b = (int)IM_F32_TO_INT8_SAT(col[2]);
        int a = (int)IM_F32_TO_INT8_SAT(col[3]);
        std::snprintf(buf, sizeof(buf), "#%02X%02X%02X%02X", r, g, b, a);

        ImGui::Spacing();
        ImGui::SetNextItemWidth(ImGui::CalcItemWidth());
        if (ImGui::InputText("##hex", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_EnterReturnsTrue))
        {
            // Parse hex; allow with or without '#', support RGB and RGBA
            const char* p = buf;
            ...
            int count = 0;
            if (std::strlen(p) >= 8)
                count = sscanf_s(p, "%02X%02X%02X%02X", &rr, &gg, &bb, &aa);
            else
                count = sscanf_s(p, "%02X%02X%02X", &rr, &gg, &bb);
            if (count >= 3)
            {
                col[0] = ImClamp((float)rr / 255.0f, 0.0f, 1.0f);
                col[1] = ImClamp((float)gg / 255.0f, 0.0f, 1.0f);
                col[2] = ImClamp((float)bb / 255.0f, 0.0f, 1.0f);
                col[3] = ImClamp((float)aa / 255.0f, 0.0f, 1.0f);
            }
        }
...
void ImAdd::ClearWidgetStates()
{
    // Clear static maps used by CustomSliderScalar (and thus SliderFloat, SliderInt)
    g_is_dragging.clear();
    g_show_input.clear();
    g_input_buffer.clear();
    g_current_value.clear();
    g_target_value.clear();

    // Clear static maps used by CheckBox
    g_color_animations.clear();

    // Clear static maps used by Button, Selectable, BeginCombo, MultiCombo, FrameRadioIcon, FrameRadio
    g_anim_states.clear();
}
