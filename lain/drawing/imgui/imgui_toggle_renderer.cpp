...
void ImGuiToggleRenderer::DrawToggle()
{
    ...
    // Draw the gray background frame
    DrawFrame(color_frame);

    // Draw the filled blue area (grab area)
    ImRect grab_bb;
    grab_bb.Min.x = _boundingBox.Min.x;
    grab_bb.Max.x = ImLerp(_boundingBox.Min.x, _boundingBox.Max.x, _animationPercent);
    grab_bb.Min.y = _boundingBox.Min.y;
    grab_bb.Max.y = _boundingBox.Max.y;

    if (grab_bb.Max.x > grab_bb.Min.x) {
        _drawList->AddRectFilled(
            ImVec2(grab_bb.Min.x, grab_bb.Min.y),
            ImVec2(grab_bb.Max.x, grab_bb.Max.y),
            ImGui::GetColorU32(_palette.Knob), // Use knob color for the filled area
            _config.FrameRounding
        );
    }
...
ImRect ImGuiToggleRenderer::CalculateKnobBounds(float radius, float animation_percent, const ImVec2& offset /*= ImVec2()*/) const
{
    ...
    const float knob_left = (animation_percent * (GetWidth() - double_radius - offset.x))
        + (animation_percent_inverse * offset.x);
    const float knob_top = _state.KnobOffset.y;
    const float knob_bottom = GetHeight() + _state.KnobOffset.y;
    const float knob_right = knob_left + double_radius;
