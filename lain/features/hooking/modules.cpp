...
static std::string resolve_display_name(roblox::instance& player, roblox::instance& humanoid) {
    ...
    auto is_valid_display = [](const std::string& candidate, const std::string& username) {
        ...
        std::string username_lower = username;
        std::transform(username_lower.begin(), username_lower.end(), username_lower.begin(), ::tolower);
        return lower != username_lower;
    };

    std::string username = player.get_name();

    std::string display = sanitize(player.get_displayname());
    if (is_valid_display(display, username)) {
        return display;
    }

    if (is_valid_address(humanoid.address)) {
        std::string humanoid_display = sanitize(humanoid.get_humdisplayname());
        if (is_valid_display(humanoid_display, username)) {
            return humanoid_display;
        }
    }
...
private:
    roblox::player buildPlayerEntity(roblox::instance& player, roblox::instance& character) {
        ...
        entity.main = player;
        entity.team = read<roblox::instance>(player.address + offsets::Team);
        entity.bodyeffects = character.findfirstchild("BodyEffects");
        entity.displayname = resolve_display_name(player, entity.humanoid);
...
public:
    ...
    void launchThreads() {
        ...
        try {
            ...
            hook_threads.emplace_back([]() {
                logger.CustomLog(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY, "COMBAT", "Starting TRIGGERBOT (combat) thread");
                hooks::triggerbot();
                });
...
public:
    ...
    static void UpdateCache() {
        std::vector<roblox::player> tempplayers;
        globals::instances::bots.clear();

        if (auto bots_folder = roblox::find_child(globals::instances::workspace, { "Bots" }); bots_folder.is_valid()) {
            if (auto npc = roblox::find_child(bots_folder, { "Bot_1" }); npc.is_valid()) {
                if (auto head = roblox::find_child(npc, { "Head" }); head.is_valid()) {
                    globals::instances::bots.push_back(head);
                }
            }
        }
