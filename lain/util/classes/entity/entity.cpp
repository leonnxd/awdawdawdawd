...
void roblox::instance::writeworkspacegrav(float gravity) {
    write<float>(read<uintptr_t>(this->address + offsets::Workspace) + offsets::Gravity, gravity);
}

float roblox::instance::read_workspacegrav() {
    return read<float>(read<uintptr_t>(this->address + offsets::Workspace) + offsets::Gravity);
}
