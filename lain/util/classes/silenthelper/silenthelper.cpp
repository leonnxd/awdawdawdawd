...
std::uint64_t get_current_input_object(std::uint64_t base_address) {
    std::uint64_t object_address = read<std::uint64_t>(base_address + offsets::InputObject);
...
void roblox::instance::rescan_silent_mouse_service() {
    if (globals::instances::mouseservice) {
        cached_input_object = get_current_input_object(globals::instances::mouseservice);
