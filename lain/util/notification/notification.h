...
#include "../../util/globals.h" // Include globals for accent_color
...
private:
    ...
    ImVec2 display_origin; // New member to store the display origin
...
public:
    void SetDisplayOrigin(ImVec2 origin) {
    ...
    void Render() {
        ...
        for (size_t i = 0; i < notifications.size(); ++i) {
            ...
            float x_pos = display_origin.x + 5.0f + notification.slide_offset;
            float y_pos = display_origin.y + notification.current_y_pos;
            ...
            if (ImGui::Begin(window_name, nullptr,
                ...
                if (notification.state == NotificationState::DISPLAYING && notification.display_start_time > 0) {
                    ...
                    ImVec4 progress_color = ImVec4(globals::misc::accent_color[0], globals::misc::accent_color[1], globals::misc::accent_color[2], notification.alpha);
                    draw_list->AddRectFilled(
                        progress_start,
                        progress_end,
...
namespace Notifications {
    inline void Info(const std::string& message, float duration = 3.0f) {
        if (!globals::misc::enable_notifications) return;
    ...
    inline void SetDisplayOrigin(ImVec2 origin) {
        g_notification_system.SetDisplayOrigin(origin);
    }
}
