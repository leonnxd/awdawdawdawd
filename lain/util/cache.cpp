...
static math::Vector3 lookvec(const math::Matrix3& rotationMatrix) {
    return { -rotationMatrix.data[2], -rotationMatrix.data[5], -rotationMatrix.data[8] };
}

static math::Vector3 rightvec(const math::Matrix3& rotationMatrix) {
    return { rotationMatrix.data[0], rotationMatrix.data[3], rotationMatrix.data[6] };
}
...
math::Matrix3 create_rotation_matrix(float pitch, float yaw) {
    math::Matrix3 result;
...
void update_movement() {
    math::Vector3 look_vector = lookvec(freecam::camera_rotation);
    math::Vector3 right_vector = rightvec(freecam::camera_rotation);
    math::Vector3 up_vector = { 0.0f, 1.0f, 0.0f };
